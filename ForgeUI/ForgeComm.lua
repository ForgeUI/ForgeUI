...
fnReceivedMessage = function(strMessage)
	...
	if tMessage.strType == "print" then
		ForgeUI:Print("ForgeComm", tostring(tBody.strText))
...
function ForgeUI:DebugMessages()
	...
	for k, v in pairs(tSentMessages) do
		self:Debug("ForgeComm", "[" .. tostring(k) .. ", " .. tostring(v.strTarget) .. ", " .. tostring(v.bSent) .."] " .. v.strMessage)
	end
	self:Debug(" --- received messages ---")
	for k, v in pairs(tReceivedMessages) do
		self:Debug("ForgeComm", "[" .. tostring(k) .. ", " .. tostring(v.strTarget) .. ", " .. tostring(v.bSent) .."] " .. tostring(v.strMessage))
	end
	self:Debug(" --- error messages ---")
	for k, v in pairs(tErrorMessages) do
		self:Debug("ForgeComm", "[" .. k .. "] " .. v)
	end
end
